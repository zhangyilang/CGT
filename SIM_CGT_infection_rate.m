...
L       = 10.^(-5:0.2:-3);  % Parameter of Possion distribution for each individual
...
% loop for SNR
num_L     = length(L);
precision   = zeros(num_method,num_L);
recall      = zeros(num_method,num_L);
...
for idx_L = 1:num_L
    ...
    for idx_itr = 1:N_T
        ...
        [x,~,T] = GenSparseVec_COVID19(L_tmp,X_mu,X_sigma,N);
        if isempty(T)
           continue
        end
        A       = generate_A(M,N,P);
    ...
    precision(:,idx_L)  = tp ./ (tp + fp + eps);
    recall(:,idx_L)     = tp ./ (tp + fn + eps);
    
    disp(['***' num2str(idx_L) '-th process took ' num2str(toc(tic1)) ' (sec.).']);
